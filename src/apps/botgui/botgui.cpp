...
BotGui::BotGui(lcm::LCM *lcmInstance, int argc, char **argv, int widthInPixels, int heightInPixels, int framesPerSecond)
    : VxGtkWindowBase(argc, argv, widthInPixels, heightInPixels, framesPerSecond), haveLaser_(false), havePath_(false), haveTruePose_(false), shouldResetStateLabels_(false), shouldClearTraces_(false), nextColorIndex_(0), lcmInstance_(lcmInstance)
{
    assert(lcmInstance_);

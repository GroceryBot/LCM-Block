                             ...
                             const SearchParams &params)
{
  ...
  for (int test_thing = 0; test_thing < 2; test_thing++)
  {
    ...
    for (unsigned int goal_choice = 0; goal_choice < options.size(); ++goal_choice)
    {
      pose_xyt_t goal = options[goal_choice];
      path.path_length = 0;
      path.utime = start.utime;
      //First node in path
      ...
      start_node->f_score = calculateHscore(*start_node, dest_node, distances, params, test_thing != 1);
      ...
      while (!pq.empty() && !isDestinationReached(*cur, dest_node))
      {
        ...
        for (unsigned int i = 0; i < neighbours.size(); i++)
        {
          if (distances(neighbours[i].x, neighbours[i].y) > params.minDistanceToObstacle)
          {
            ...
            if (i >= 4)
            {
              n->g_score += (sqrt(2)) * metersPerCell;
            }
            auto iter = open_set.find(n);
            if (iter == open_set.end())
            {
              n->f_score = n->g_score + calculateHscore(*n, dest_node, distances, params, test_thing != 1);
              open_set.insert(n);
              pq.push(n);
            ...
            else
            {
              ...
              {
                (*iter)->p = cur;
                (*iter)->f_score = n->g_score + calculateHscore(*(*iter), dest_node, distances, params, test_thing != 1);
